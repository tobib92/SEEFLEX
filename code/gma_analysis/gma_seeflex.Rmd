...
load("data/gma/data.rda", verbose=TRUE)
...
scatterD3(data = Proj5.df, x = LD2, y = LD1, fixed = TRUE,
          col_var = operator, colors = c17_corp.vec, ellipses = TRUE,
          symbol_var = t.curr, symbols = NULL, labels_size = 100,
...
scat <- onRender(
  ...
  "
function(el, x) {
  // Increase axis labels (x and y labels)
  //d3.select(el).selectAll('.xlab, .ylab').style('font-size', '20px');

  // Increase tick (scale) labels.
  // If scatterD3 uses the d3-axis, tick labels are usually contained in groups with a common class.
  // Here we try '.d3-axis text' which is a common structure.
  //d3.select(el).selectAll('.d3-axis text').style('font-size', '16px');

  // Increase legend labels
  //d3.select(el).selectAll('.legend text').style('font-size', '20px');

  // ---
  // As an alternative: If you want every text element to be the same (e.g., if
  // you really want all text to be larger), you could use:
  d3.select(el).selectAll('text').style('font-size', '20px');
  // ---
}
  "
)

saveWidget(scat, file = "plot.html", selfcontained = TRUE)
...
weights <- t(weights)
gma.plot.weights(weights, feature.names = feature.names, dim = 1:4)
